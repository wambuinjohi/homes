...
CREATE TABLE IF NOT EXISTS public.sms_usage_logs (
  ...
  recipient_phone text NOT NULL, -- Masked phone number
  message_content text, -- Message length or preview only
...
CREATE INDEX IF NOT EXISTS idx_sms_usage_logs_landlord_date ON public.sms_usage_logs(landlord_id, created_at DESC);