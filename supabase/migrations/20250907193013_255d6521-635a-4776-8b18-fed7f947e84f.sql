...
SELECT 
  ...
  COALESCE(pa.amount_paid_allocated, 0)::numeric as amount_paid_allocated,
  COALESCE(pd.amount_paid_direct, 0)::numeric as amount_paid_direct,
  GREATEST(COALESCE(pa.amount_paid_allocated, 0), COALESCE(pd.amount_paid_direct, 0))::numeric as amount_paid_total,
  GREATEST(i.amount - GREATEST(COALESCE(pa.amount_paid_allocated, 0), COALESCE(pd.amount_paid_direct, 0)), 0)::numeric as outstanding_amount,
  -- Computed status
  CASE 
    WHEN GREATEST(COALESCE(pa.amount_paid_allocated, 0), COALESCE(pd.amount_paid_direct, 0)) >= i.amount THEN 'paid'
...
-- Payment allocations aggregation
LEFT JOIN (
  SELECT 
    invoice_id,
    SUM(amount) as amount_paid_allocated
